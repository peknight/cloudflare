...
trait StringOptionParamInstances:
  given codecStringOptionParam[F[_], S](using CodecConfig, Monad[F], ObjectType[S], NullType[S], StringType[S], Show[S])
  : Codec[F, S, Cursor[S], StringOptionParam] =
    Codec.derived[F, S, StringOptionParam]
  given showStringOptionParam: Show[StringOptionParam] = show.derived[StringOptionParam]
